...
sequenceDiagram
    ...
    Customer->>Merchant: Place order

    rect rgb(245,245,245)
        Note over Merchant,Gateway: Invoice validation

        Merchant->>Gateway: POST /invoice/try
        activate Gateway

        Gateway-->>Merchant: result=true

        deactivate Gateway
    end

    rect rgb(245,245,245)
        Note over Merchant,Gateway: Invoice creation

        Merchant->>Gateway: POST /invoice/create
        activate Gateway

        Gateway-->>Merchant: invoiceId, paymentUrl
    ...
    Note over Customer,Gateway: Customer completes payment

    critical Payment completed

        Gateway-)Merchant: Callback: InvoicePaid
        activate Merchant

        Merchant-->>Gateway: HTTP 200 OK

        deactivate Merchant

    end

    Merchant-->>Customer: Deliver goods/services
```
...
sequenceDiagram
    ...
    rect rgb(245,245,245)
        Note over Merchant,Gateway: Payment validation
    ...
    rect rgb(245,245,245)
        Note over Merchant,Gateway: Funds authorization

        Merchant->>Gateway: POST /invoice/hold
        activate Gateway

        Gateway-->>Merchant: holdId, paymentUrl
    ...
    Note over Customer,Gateway: Customer confirms payment

    critical Funds successfully blocked

        Gateway-)Merchant: Callback: FundsHeld
        activate Merchant
    ...
    Note over Merchant: Order processing

    rect rgb(245,245,245)
        Note over Merchant,Gateway: Capture authorized funds

        Merchant->>Gateway: POST /invoice/charge
        activate Gateway

        Gateway-->>Merchant: charge accepted
    ...
    critical Capture completed
        ...
        Gateway-)Merchant: Callback: FundsCaptured
        activate Merchant
    ...
    Merchant-->>Customer: Order completed
```
