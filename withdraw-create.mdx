...
sequenceDiagram
    ...
    rect rgb(245,245,245)
        Note over Merchant,Gateway: Payout validation

        Merchant->>Gateway: POST /withdraw/try
        Gateway-->>Merchant: result=true
    end

    rect rgb(245,245,245)
        Note over Merchant,Gateway: Account validation

        Merchant->>Gateway: POST /check_account
        Gateway-->>Merchant: accountAvailable=true
    end

    rect rgb(245,245,245)
        Note over Merchant,Gateway: Create payout

        Merchant->>Gateway: POST /withdraw/create
        activate Gateway

        Gateway-->>Merchant: payoutId
    ...
    loop Until final status received
        ...
        alt Search by payoutId
            ...
            Merchant->>Gateway: GET /withdraw/status
            Gateway-->>Merchant: status

        else Search by merchant payment number

            Merchant->>Gateway: GET /withdraw/shop_payment_status
            Gateway-->>Merchant: status
    ...
    Note over Merchant,Gateway: Final status = SUCCESS / FAILED
```
